...
import tqdm
# from synthesize.LocalStableDiffusionPipeline_Guide_v4 import LocalStableDiffusionPipeline
from synthesize.LocalStableDiffusionPipeline_Guide_CAD_Div import LocalStableDiffusionPipeline
...
def generate_images(data_loader, pipe, output_dir, image_size, images_per_class, prompt_strength, inference_steps):
    ...
    while any(class_indices[class_name] < images_per_class for class_name in class_indices):
        for batch_idx, (images, prompts, negative_prompts, class_names) in tqdm.tqdm(enumerate(data_loader), total=len(data_loader), desc="Generating images"):
            ...
            for class_name in class_names:  # class_names 리스트에서 class_name 추출
                # noise_scale=((1+batch_idx)/len(data_loader))*0.06*2
                noise_scale=(float((1+class_indices[class_name]))/images_per_class)*0.06*2
                # print(f"Noise scale for {batch_idx}:", noise_scale)
            with torch.no_grad():
...
def main(args):
    ...
    generate_images(data_loader, pipe, output_dir, image_size, images_per_class, prompt_strength, inference_step)
