...
def generate_images(data_loader, pipe, output_dir, image_size, images_per_class, classifier_scale, prompt_strength, inference_steps):
    ...
    while any(class_indices[class_name] < images_per_class for class_name in class_indices):
        for batch_idx, (images, prompts, negative_prompts, class_names) in tqdm.tqdm(enumerate(data_loader), total=len(data_loader), desc="Generating images"):
            ...
            with torch.no_grad():
                ...
                resized_images =[img.resize(image_size, Image.LANCZOS) for img in generated_images] #
...
def main(args):
    ...
    hidden_dim = 512
    if args.subset in ["imagenet-woof", "cifar10", "imagenette"]:
        num_classes = 10  # CIFAR-100 클래스 수
    elif args.subset in ["tiny-imagenet"]:
        num_classes = 200  # CIFAR-100 클래스 수
    else:
    ...
    generate_images(data_loader, pipe, output_dir, image_size, images_per_class, classifier_scale, prompt_strength, inference_step)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mipc", type=int, default=100, required=True, help="Number of images to generate per class")
    parser.add_argument("--num_inference_steps", type=int, default=20, help="Number of denoising steps")
    parser.add_argument("--prompt_strength", type=float, default=1.1, help="Classifier guidance scale")
