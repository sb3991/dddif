...
from diffusers import StableDiffusionPipeline, EulerDiscreteScheduler, UNet2DConditionModel, StableDiffusionImg2ImgPipeline, DDIMScheduler, StableDiffusion3Pipeline, DDPMScheduler, LMSDiscreteScheduler, PNDMScheduler
import tqdm
import numpy as np
    ...
    while any(class_indices[class_name] < images_per_class for class_name in class_indices):
        for batch_idx, (images, prompts, negative_prompts, class_names) in tqdm.tqdm(enumerate(data_loader), total=len(data_loader), desc="Generating images"):
            ...
            with torch.no_grad():
                ...
                resized_images = [img.resize(image_size, Image.LANCZOS) for img in generated_images] #generated_images
...
def main(args):
    ...
    cads_scheduler = CADSPNDMScheduler(
