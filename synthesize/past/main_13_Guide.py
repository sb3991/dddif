...
class TimeEmbedding(nn.Module):
    ...
    def forward(self, t):
        ...
        # t 텐서를 linear1 가중치와 동일한 디바이스로 이동
        t = t.to(self.linear1.weight.device)
...
class ClassifierGuidedStableDiffusionPipeline(StableDiffusionPipeline):
    ...
    #             grad = torch.autograd.grad(class_loss.sum(), sample, retain_graph=True)[0]
    ...
    #             # 클래스 가이던스 적용
    #             model_output = model_output - gradient_scale * (beta_prod_t ** 0.5) * grad
    ...
    #         if k in ['generator', 'prev_sample', 'noise_pred'] and v is not None
    #     }

    #     # 스케줄러 스텝의 결과를 직접 반환
    #     return self.scheduler.step(model_output, timestep, sample, **scheduler_kwargs)
    def step(self, model_output, timestep, sample, **kwargs):
        ...
        if self.classifier is not None:
            ...
            if gradient_scale > 0:
                ...
                if class_labels is not None:
                    ...
                    model_output = model_output - gradient_scale * (beta_prod_t ** 0.5) * grad
            # else:
                # gradient_scale이 0이거나 음수일 때는 그래디언트 계산을 하지 않습니다
                # with torch.no_grad():
                #     class_scores = self.classifier(sample, timestep)
                ...
                # print(f"Grad norm: {grad.norm().item()}")
                # print(f"Guidance norm: {guidance.norm().item()}")
                # print(f"Model output change: {(model_output - model_output_before).norm().item()}")
                # print(f"Sample change: {(sample - original_sample).norm().item()}")
        # 스케줄러에 전달할 인자 필터링
        with torch.no_grad():
            scheduler_kwargs = {
                k: v for k, v in kwargs.items() 
        ...
        # 스케줄러 스텝
        # scheduler_output = self.scheduler.step(model_output, timestep, sample, **kwargs)
        
        # print(f"Scheduler step change: {(scheduler_output.prev_sample - sample).norm().item()}")
    ...
    def __call__(self, *args, **kwargs):
        ...
        # 입력 이미지를 인코더에 넣어 초기 latent 벡터 생성
        image = kwargs.get('image')
        if image is None:
            raise ValueError("Input image is required for generating latents")
        ...
        # latents 생성
        with torch.no_grad():
            latents = self.vae.encode(image.to(self.device).half()).latent_dist.sample()
        ...
        for i, t in enumerate(self.scheduler.timesteps):
            ...
            # 스케줄러 스텝
            step_output = self.step(noise_pred, t, latents, **step_kwargs)
            latents = step_output.prev_sample

            # 디버깅: latents의 통계 출력
            # print(f"Step {i}, Latents mean: {latents.mean().item():.4f}, std: {latents.std().item():.4f}")

        # VAE를 사용하여 latents를 이미지로 디코딩
        with torch.no_grad():
