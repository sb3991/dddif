...
class ExpLocalStableDiffusionPipeline(StableDiffusionPipeline):
    ...
    def __call__(
        ...
        # hyperparameters for prompt embedding interpolation
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if method is None:
                    ...
                    prompt_embeds = self.interpolate_prompt(i, num_inference_steps, tau1, tau2, noise_scale, mixing_factor, rescale, og_prompt_embeds)
                elif method == 'stopgradient':
                    pass
                else:
                    raise ValueError
                ...
                if do_classifier_free_guidance:
                    ...
                    noise_cond.append(torch.sum(noise_pred_text**2, dim=(1, 2, 3)))
                    noise_uncond.append(torch.sum(noise_pred_uncond**2, dim=(1, 2, 3)))
                    
                    noise_pred_text = noise_pred_text - noise_pred_uncond

                    noise_diff_mag.append(torch.sum(noise_pred_text**2, dim=(1, 2, 3)))
        ...
        # print('====')
        noise_diff_mag = torch.stack(noise_diff_mag, dim=1)
        noise_cond = torch.stack(noise_cond, dim=1)
        noise_uncond = torch.stack(noise_uncond, dim=1)
        # print('noise_diff_mag: ', noise_diff_mag)
        # print('noise_cond: ', noise_cond)
        # print('noise_uncond: ', noise_uncond)
        # print('cos_between_uncond_and_diff: ', cos_between_uncond_and_diff)
        # print('====')
        # exit()
    ...
    def corrupt_text_cads(self, cur_t, num_inference_steps, t1, t2, noise_scale, mixing_factor, rescale, prompt_embeds):
        ...
        t = 1.0 - max(min(cur_t / num_inference_steps, 1.0), 0.0) # Algorithms assumes we start at 1.0 and go to 0.0
        gamma = cads_linear_schedule(t, t1, t2)

        num_imgs_per_prompt = int(prompt_embeds.shape[0] / 2)
        uncond_prompt_embeds = add_noise(prompt_embeds[0], gamma, noise_scale, mixing_factor, rescale)
        cond_prompt_embeds = add_noise(prompt_embeds[7], gamma, noise_scale, mixing_factor, rescale)

        uncond_prompt_embeds = uncond_prompt_embeds.unsqueeze(dim=0)
        uncond_prompt_embeds = uncond_prompt_embeds.repeat(num_imgs_per_prompt, 1, 1)
        cond_prompt_embeds = cond_prompt_embeds.unsqueeze(dim=0)
        cond_prompt_embeds = cond_prompt_embeds.repeat(num_imgs_per_prompt, 1, 1)
        
        return torch.cat([uncond_prompt_embeds, cond_prompt_embeds], dim=0)
    
    def interpolate_prompt(self, cur_t, num_inference_steps, t1, t2, noise_scale, mixing_factor, rescale, prompt_embeds):
        def cads_linear_schedule(t, tau1, tau2):
            """ CADS annealing schedule function """
            if t <= tau1:
                    return 1.0
        ...
        def interpolate_embeds(prompt_embeds, gamma, noise_scale, psi, rescale=False):
            """ CADS adding noise to the condition
            ...
            rescale (bool): Rescale the condition
            """
            uncond_prompt_embeds = prompt_embeds[0]
            cond_prompt_embeds = prompt_embeds[7]

            interpolated_prompt_embeds = gamma * cond_prompt_embeds + (1-gamma) * uncond_prompt_embeds
            
            return interpolated_prompt_embeds, uncond_prompt_embeds
        
        t = 1.0 - max(min(cur_t / num_inference_steps, 1.0), 0.0) # Algorithms assumes we start at 1.0 and go to 0.0
        gamma = cads_linear_schedule(t, t1, t2)

        num_imgs_per_prompt = int(prompt_embeds.shape[0] / 2)
        interpolated_prompt_embeds, uncond_prompt_embeds = interpolate_embeds(prompt_embeds, gamma, noise_scale, mixing_factor, rescale)

        uncond_prompt_embeds = uncond_prompt_embeds.unsqueeze(dim=0)
        uncond_prompt_embeds = uncond_prompt_embeds.repeat(num_imgs_per_prompt, 1, 1)
        interpolated_prompt_embeds = interpolated_prompt_embeds.unsqueeze(dim=0)
        interpolated_prompt_embeds = interpolated_prompt_embeds.repeat(num_imgs_per_prompt, 1, 1)
        
        return torch.cat([uncond_prompt_embeds, interpolated_prompt_embeds], dim=0)
