...
from diffusers import StableDiffusionPipeline, EulerDiscreteScheduler, UNet2DConditionModel, StableDiffusionImg2ImgPipeline, DDIMScheduler, StableDiffusion3Pipeline, DDPMScheduler, LMSDiscreteScheduler, PNDMScheduler, DPMSolverMultistepScheduler
import tqdm
import numpy as np
...
class CADSDPMSolverMultistepScheduler(DPMSolverMultistepScheduler):
    ...
    def step(self, model_output, t, sample, **kwargs):
        ...
        noise = self.noise_scale * torch.randn_like(sample)
        modified_latents = self.add_noise_CADS(sample, noise, t)
        return super().step(model_output, t, modified_latents, **kwargs)
...
# class CADSPNDMScheduler(PNDMScheduler):
#     def __init__(self, tau1=0.8, tau2=0.99, noise_scale=0.15, psi=0.9, rescale=True, **kwargs):
#         super().__init__(**kwargs)
#         self.tau1 = tau1
#         self.tau2 = tau2
...
#     def add_noise_CADS(self, latents, noise, t):
#         """Add noise to embeddings based on the CADS strategy and current timestep."""
#         gamma = self.cads_linear_schedule(t / 950)  # PNDM typically uses more steps, adjusting for that
#         # print("t", t, "gamma", gamma)
#         y_noisy = gamma * latents + (1 - gamma) * noise
#         if self.rescale:
#             y_mean, y_std = torch.mean(latents), torch.std(latents)
...
def main(args):
    ...
    cads_scheduler = CADSDPMSolverMultistepScheduler(
