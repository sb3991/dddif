...
def selector_1(n, model, images, labels, size, m=5):
    with torch.no_grad():
        # [mipc, m, 3, 224, 224]
        images = images.cuda()
        s = images.shape
        ...
        rand_indices = torch.randperm(s[0])[:n]

        # [mipc, 3, 224, 224]
        selected_images = images[0, rand_indices]  # Assuming m is always greater than 0 and selecting from first m set

    torch.cuda.empty_cache()
    return selected_images.detach()
...
def min_max_scale(tensor):
    ...
    if max_val.item() - min_val.item() == 0:
        return tensor - min_val  # avoid division by zero
    return (tensor - min_val) / (max_val - min_val)



def selector_2(n, model, images, labels, size, selected_images, m=5):
    with torch.no_grad():
        # [mipc, m, 3, 224, 224]
        images = images.cuda()
        s = images.shape
        ...
        # Flatten images for cosine similarity calculation
        flattened_images = images.view(images.size(0), -1)
        flattened_selected = selected_images.view(selected_images.size(0), -1).cuda()
        
        # Calculate cosine similarity
        similarity = F.cosine_similarity(flattened_images.unsqueeze(1), flattened_selected.unsqueeze(0), dim=2)
        
        # Get the maximum similarity for each image
        max_similarity, _ = similarity.max(dim=1)
        # max_similarity, _ = similarity.min(dim=1)
        # max_similarity = similarity.mean(dim=1)
        
        # Reshape max_similarity to [m, mipc]
        max_similarity = max_similarity.reshape(m, s[0])
        
        # Select the image with the lowest similarity from each group
        index = torch.argmin(max_similarity, dim=0)
        
        # Get the selected images
        sa = images.shape
        images = images.reshape(m, s[0], sa[1], sa[2], sa[3])
        selected = images[index, torch.arange(s[0])]
        
        # Get the n images with the lowest overall similarity
        similarity_scores = max_similarity[index, torch.arange(s[0])]
        indices = torch.argsort(similarity_scores)[:n]
    
    torch.cuda.empty_cache()
    return selected[indices].detach()
...
def load_model_2(model_name="resnet18", dataset="cifar10", pretrained=True, classes=[]):
    def get_model(model_name="resnet18"):
        if "conv" in model_name:
            if dataset in ["cifar10", "cifar100"]:
                size = 32
    ...
    if pretrained:
        if dataset in [
            ...
            checkpoint = torch.load(
                "/home/sb/link/DD_DIF/exp/imagenet-woof_resnet18_f2_mipc300_ipc50_cr5_Guided_random/saved_models/fintuned_w_DIF_imagenet-woof.pth"
                # ,map_location="cpu"
            )
